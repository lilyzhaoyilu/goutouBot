...
class ApexNiko extends AppCommand {
  code = 'niko'; // 只是用作标记
  trigger = 'niko'; // 用于触发的文字
  help = '发送`.apex niko`就可以啦~'; // 帮助文字
  intro = '什么时候会有intro';
  func: AppFunc<BaseSession> = async (session) => {
    const msg_id = await GoutouCard.sendQueringCard(session);
    const card = await Streamer.assembleStreamerCard('niko', session);
    await normalSendOutCardWrapper(session, card, msg_id);
  };
}

export const apexNiko = new ApexNiko();