...
class ApexTianyao extends AppCommand {
  code = 'tianyao'; // 只是用作标记
  trigger = 'tianyao'; // 用于触发的文字
  help = '发送`.apex tianyao`就可以啦~'; // 帮助文字
  intro = '什么时候会有intro';
  func: AppFunc<BaseSession> = async (session) => {
    const msg_id = await GoutouCard.sendQueringCard(session);
    const card = await Streamer.assembleStreamerCard('tianyao', session);
    await normalSendOutCardWrapper(session, card, msg_id);
  };
}

export const apexTianyao = new ApexTianyao();