  ...
  if (e.channel_type === 'PERSON' && e.extra?.author?.username && e.extra?.author?.bot === false) {
    try {
      ...
      if (e.extra?.last_msg_content === '[卡片]' && e.author_id) {
        bot.API.directMessage.create(9, e.author_id?.toString(), undefined, '用**电脑**直接戳[邀请链接](https://www.kaiheila.cn/app/oauth2/authorize?id=11038&permissions=268435455&client_id=dZ69aXhULKhX5krk&redirect_uri=&scope=bot)。手机端暂时不能邀请机器人。需要你拥有邀请服务器的管理权限。')
      } else if (e.content === '1') {
        bot.API.directMessage.create(9, e.author_id?.toString(), undefined, '2')
      } else if (e.content.includes('apex')) {
        bot.API.directMessage.create(9, e.author_id?.toString(), undefined, '你好，请在有狗头机器人的频道里发送 .apex 查看狗头机器人的菜单。暂时不支持私信查询。如果有什么问题可以私信。')
      } else {
        bot.API.directMessage.create(9, e.author_id?.toString(), undefined, '狗头已经收到了消息，作者在线了就会回复。作者在线的时间一般是工作日的早上和周末的凌晨-下午。  可以在有狗头机器人的频道里发送 .apex 查看狗头机器人的菜单。暂时不支持私信查询。')
      }
    } catch (err) {
